...
const CountrySelectDropdown = ({ value, onChange, disabled }) => {
  ...
  return (
    <div ref={rootRef} className="relative min-w-[200px] flex-[0_1_220px]">
      ...
      >
        {selected.flagValue ? (
          <CountryFlag value={selected.flagValue} size="xs" mode="image" />
        ) : (
      ...
      {open && (
        ...
        >
          {NEWSLETTER_COUNTRIES.map((c) => (
            <li key={c.value || "empty"} role="option" aria-selected={value === c.value}>
              <button
                type="button"
                onClick={() => {
                  onChange(c.value);
                ...
                className={`flex w-full items-center gap-2.5 px-3 py-2.5 text-left text-sm transition hover:bg-zinc-800 ${value === c.value ? "bg-purple-900/30 text-violet-200" : "text-zinc-200"
                  }`}
              >
                {c.flagValue ? (
                  <CountryFlag value={c.flagValue} size="xs" mode="image" />
                ) : (
      ...
      <div className="mx-auto flex max-w-3xl flex-col items-center gap-6 text-center">
        <div className="space-y-2">
          ...
          <p className="text-sm text-zinc-500">
            Subscribe for curated deals, new arrivals, and region-specific alerts.
          </p>
        </div>
...
const Footer = () => {
  ...
  const socialLinks = [
    { icon: YoutubeIcon, label: "YouTube", href: "https://www.youtube.com/@ANIMEPROPHECY22", color: "hover:text-red-500" },
    { icon: PinIcon, label: "Pinterest", href: "https://pin.it/Mmw0KNf9R", color: "hover:text-rose-400" },
    { icon: FacebookIcon, label: "Facebook", href: "https://www.facebook.com/share/1FQ7qwjUj4/", color: "hover:text-sky-400" },
    { icon: InstagramIcon, label: "Instagram", href: "https://www.instagram.com/anime.prophecy_official?igsh=MWljb21tb2xzeGJ6Yw==", color: "hover:text-pink-400" },
  ];
    ...
    <footer className="bg-zinc-950 border-t border-zinc-800">
      ...
      <div className="max-w-7xl mx-auto px-4 py-14">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <svg viewBox="0 0 24 24" className="h-8 w-8 text-purple-400" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d="M12 2l1.9 5.8L20 9.7l-5.3 3.8L16.8 20 12 16.7 7.2 20l2.1-6.5L4 9.7l6.1-1.9Z" />
              </svg>
              <h3 className="text-xl font-bold text-white">Prophecy Hub</h3>
            </div>
            <p className="text-zinc-400 text-sm leading-6">
              A premium anime commerce experience with regional storefronts, curated product discovery, and a dark cinematic interface.
            </p>
            <p className="text-zinc-500 text-xs">© 2026 Prophecy Hub. All rights reserved.</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2">
              {footerLinks.company.map((link) => (
                <li key={link.label}>
                  <Link to={link.to} className="text-zinc-400 hover:text-purple-400 transition-colors text-sm">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
            ...
            <div className="flex gap-3">
              {socialLinks.map((social) => (
                <a key={social.label} href={social.href} target="_blank" rel="noopener noreferrer"
                  className={`text-zinc-400 transition-colors ${social.color}`} title={social.label}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-8 border-t border-zinc-800">
          <div className="space-y-1">
            ...
            <a href={`mailto:${CONTACT_EMAIL}`} className="text-white font-semibold hover:text-purple-400 transition-colors">{CONTACT_EMAIL}</a>
          </div>
          <div className="space-y-1">
            <p className="text-zinc-500 text-xs">LOCATION</p>
            <p className="text-white font-semibold">Anime Merchandise Hub, Global</p>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-zinc-800 bg-zinc-900/40 py-5">
        <div className="max-w-7xl mx-auto px-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between text-xs text-zinc-500">
          <p>Prophecy Hub is not affiliated with any anime studios or production companies. Product links may generate affiliate commissions.</p>
