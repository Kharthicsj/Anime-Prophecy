...
import { FiX, FiRefreshCw, FiSearch, FiDownload, FiCalendar, FiList, FiTrash2, FiArrowLeft } from "react-icons/fi";
import { FaPinterest } from "react-icons/fa";
import apiClient from "../../services/apiClient";
import FilterBar from "../common/FilterBar";

const PinterestExportModal = ({ onClose, onExportComplete }) => {
	const [selectedFilters, setSelectedFilters] = useState({
		...
		status: "All Statuses",
		sort: "-createdAt",
        pinterestExported: "All", // Added filter
	});
    ...
    // Previous Exports State
    const [showPreviousExports, setShowPreviousExports] = useState(false);
	...
	const fetchPage = useCallback(async (page, reset = false) => {
		...
		try {
			...
			console.error("Failed to fetch products for Pinterest modal", err);
		} finally {
			setIsLoading(false);
    ...
    const fetchPreviousExports = useCallback(async () => {
        ...
        try {
            const res = await apiClient.get('/products/admin/pinterest-export');
    ...
    useEffect(() => {
        if (showPreviousExports) {
            fetchPreviousExports();
        }
    }, [showPreviousExports, fetchPreviousExports]);

	useEffect(() => {
		const el = sentinelRef.current;
		if (!el) return;
		const observer = new IntersectionObserver((entries) => {
			if (entries[0].isIntersecting && hasMore && !isLoading && !showPreviousExports) {
				fetchPage(currentPage + 1);
			}
		}, { rootMargin: "200px" });
		observer.observe(el);
		return () => observer.disconnect();
	}, [hasMore, isLoading, currentPage, fetchPage, showPreviousExports]);
    ...
    const convertToCSV = (dataList, forcedScheduleDate) => {
        ...
        // Exact Pinterest Format Headers
        ...
        const rows = dataList.map(p => {
            ...
            // Build Description (max 500 chars including tags)
	...
	const handleExport = async () => {
		...
		try {
			...
			const res = await apiClient.post("/products/admin/pinterest-export", { 
                ids: productIds,
			...
			setTimeout(() => {
                setSuccessMsg("");
                if(!showPreviousExports) fetchPage(1, true); // Refresh products to show badge
            }, 3000);
		} catch (err) {
    ...
    const handleDeleteExport = async (id) => {
        const confirmReset = window.confirm("Do you want to reset the products' exported status to 'Not Exported' for this record?");
        try {
            await apiClient.delete(`/products/admin/pinterest-export/${id}?resetProducts=${confirmReset}`);
            fetchPreviousExports();
            if(onExportComplete) onExportComplete(); // Tell parent to reload
            fetchPage(1, true); // Reload local list
        } catch (err) {
		...
		>
			<div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-7xl h-[92vh] flex flex-col shadow-2xl overflow-hidden">
					...
					<div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white flex items-center gap-2.5">
                                <FaPinterest className="text-red-500 text-2xl" /> {showPreviousExports ? "Previous Exports" : "Pinterest Bulk Export"}
                            </h2>
                            <p className="text-sm text-zinc-400 mt-1">
                                {showPreviousExports ? "View and manage your previously generated Pinterest CSV files." : "Select products across multiple filters, then export them to a CSV file."}
                            </p>
                        </div>
					</div>
                    <div className="flex items-center gap-4">
                        {!showPreviousExports && (
                            <button
                                ...
                                className="cursor-pointer flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold border transition-all bg-zinc-800 border-zinc-700 text-zinc-300 hover:border-zinc-500"
                            >
				...
				<div className="overflow-y-auto flex-1 bg-black/20">
                    {showPreviousExports ? (
                        <div className="p-6">
                            <h3 className="text-lg font-bold text-white mb-4">Previous Exports</h3>
                            {isLoadingExports ? (
                                <p className="text-zinc-500">Loading previous exports...</p>
                                    ...
                                    {previousExports.map(exp => (
                                        <div key={exp._id} className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 flex flex-wrap gap-4 items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                >
                                                    {downloadingExportId === exp._id ? <FiRefreshCw className="animate-spin" /> : <FiDownload />} Download CSV
                                                </button>
                                                <button 
                                    ...
                                    <div className="flex flex-wrap items-center gap-4">
                                        ...
                                        {selectedCount > 0 && (
                                            <div className="flex items-center gap-3 ml-2 border-l border-zinc-700 pl-4">
                                                ...
                                                >
                                                    <FiDownload className="text-lg" /> {isExporting ? "Exporting..." : `Export CSV (${selectedCount})`}
                                                </button>
                                            </div>
                                        )}
                                    </div>
