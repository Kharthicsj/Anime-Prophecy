...
const PrivateProductsModal = ({ onClose, onPrivacyChanged }) => {
	...
	const [isLoading, setIsLoading] = useState(false);
	const [totalProducts, setTotalProducts] = useState(0);
	// selectedIds tracks IDs of currently loaded products that are checked
	const [selectedIds, setSelectedIds] = useState(new Set());
	// selectAllMode = true means ALL records in DB (matching filters) are selected
	...
	const toggleSelect = (id) => {
		// Toggling a card deactivates selectAllMode
		setSelectAllMode(false);
		setSelectedIds(prev => {
	...
	// Select All: first click → select all LOADED; second click → fetch ALL DB IDs; third click → deselect all
	const handleSelectAll = async () => {
		if (selectAllMode) {
			// Already in DB-wide select-all → deselect everything
			setSelectAllMode(false);
			setSelectedIds(new Set());
			return;
		}
		if (selectedIds.size === products.length && products.length > 0) {
			// All loaded selected → escalate to DB-wide select-all
			setIsLoading(true);
			try {
	...
	// Shared bulk update — isActive: false = private, true = active
	// Uses a single PATCH endpoint that does one DB updateMany regardless of
	// how many products are selected (rate-limit safe).
	const handleBulkUpdate = async (isActive) => {
		...
		try {
			let body;
			if (selectAllMode) {
				// Send the current filters to the backend — it applies updateMany
				// with those filters server-side. Zero IDs need to be transmitted.
				body = {
					isActive,
					applyToAll: true,
					filters: {
				...
				// Send the specific IDs — backend does one updateMany with $in filter
	...
	// Determine mixed states among selected for smart button labels
